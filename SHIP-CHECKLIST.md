...
- [ ] M10 runtime MEASURED (Bronson 2026-08-12): ~15 min — 5-10 under the 20-25 mandate. Lengthening levers, structural not padded, awaiting his pick: (a) sequence the siege — hide the HQ objective until the gate batteries fall (the M7 forts→HQ chain) and grow the batteries 4→6; (b) Krauss repairs — red engineer crews inside the city keep the wall honest; (c) a counterattack sortie when the batteries fall — forces a defense round trip mid-siege; (d) evac becomes staged (hold the pass while transports cycle) — only if the run ending feels thin
